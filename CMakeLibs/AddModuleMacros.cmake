...
MACRO(LU_SETUP_EXPORT_DEFINITIONS TARGET_NAME)
    STRING(TOUPPER ${TARGET_NAME} BUILD_DLL_PREFIX)
    GET_TARGET_PROPERTY(TARGET_TYPE ${TARGET_NAME} TYPE)

    IF(TARGET_TYPE STREQUAL "SHARED_LIBRARY")
        IF(WIN32)
            TARGET_COMPILE_DEFINITIONS(${TARGET_NAME} PRIVATE "${BUILD_DLL_PREFIX}_API=__declspec(dllexport)")
            TARGET_COMPILE_DEFINITIONS(${TARGET_NAME} INTERFACE "${BUILD_DLL_PREFIX}_API=__declspec(dllimport)")
        ELSE()
            TARGET_COMPILE_DEFINITIONS(${TARGET_NAME} PRIVATE "${BUILD_DLL_PREFIX}_API=__attribute__((visibility(\"default\")))")
            TARGET_COMPILE_DEFINITIONS(${TARGET_NAME} INTERFACE "${BUILD_DLL_PREFIX}_API=")
        ENDIF()
    ELSE()
        TARGET_COMPILE_DEFINITIONS(${TARGET_NAME} PUBLIC "-D${BUILD_DLL_PREFIX}_API=")
    ENDIF()
ENDMACRO()
...
MACRO(LU_ADD_LIBRARY)
    ...
    LU_SETUP_EXPORT_DEFINITIONS(${ARG_TARGET})
